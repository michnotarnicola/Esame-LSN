...
// Instantaneous values (commented after the equilibration phase)
  ofstream Vinst ;
  ofstream Pinst ;
  Vinst.open("V_inst.dat",ios::app);
  Pinst.open("P_inst.dat",ios::app);
  Vinst << walker[iv]/(double)npart + vtail << endl;
  Pinst << rho * temp + (walker[iw] + (double)npart * ptail) / vol <<endl ;
  Vinst.close();
  Pinst.close();
}
...
//g(r)
    for(int ig=igofr; ig <n_props ; ig++) {
      Gofr << setw(wd) << iblk <<  setw(wd) << r[ig-igofr]<< setw(wd) << stima_gdir[ig] << setw(wd) << glob_av[ig]/(double)iblk << setw(wd) << err_gdir[ig] << endl;
    }

    if(iblk==nblk) {
      for(int ig=igofr; ig <n_props ; ig++) Gave << r[ig-igofr]<< setw(wd) << glob_av[ig]/(double)nblk << setw(wd) << err_gdir[ig] << endl;
    }
